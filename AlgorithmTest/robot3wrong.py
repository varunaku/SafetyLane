...
positions = np.array([[15, 35], [10, 35], [10, 40]]) #first test case 
goal = [10, 40]
...
for step in range(steps):
    ...
    if uwb_distance12 < 5.1 and uwb_distance12 > 4.9 and not correctPos2:
        ...
        correctPos2 = True
    if uwb_distance13 < 10.01 and uwb_distance13 > 9.9 and not correctPos3:
        print("CORRECT POS ROBOT 3")
        correctPos3 = True
    if correctPos2 and uwb_distance13 < 10.01 and uwb_distance13 > 9.9 and uwb_distance23 < 5.1 and uwb_distance23 > 4.9:
    ...
    if not correctPos23 and not correctPos2:
        if dfound_x == 7:
            # print("dfound becoming random", dfound_x)
            #robot 2 selects random x move, then random y move 
            movement = np.random.choice([-1, 0, 1], size=2)                
        ...
        if np.abs(new_uwb_distance12 - 5) + np.abs(new_uwb_distance23 - 5) < np.abs(uwb_distance12 - 5) + np.abs(uwb_distance23 - 5):
            ...
            dfound_x = 7
            dfound_y = 7
    #robot 3 wrong, 2+1 correct
    elif not correctPos23:
        if dfound_x == 7:
            # print("dfound becoming random", dfound_x)
            #robot 2 selects random x move, then random y move 
            movement = np.random.choice([-1, 0, 1], size=2)                
            ...
            positions[0][0] = positions[0][0] + movement[0]
            positions[0][1] = positions[0][1] + movement[1]
        else: 
            # print("dfound continuing", dfound_x, dfound_y)
            positions[0][0] = positions[0][0] + dfound_x
            positions[0][1] = positions[0][1] + dfound_y

        #we guessed a direction to move, now we check
        new_uwb_distance13 = np.sqrt( np.square(np.abs(positions[2][0] - positions[0][0])) + np.square(np.abs(positions[2][1] - positions[0][1])))
        new_uwb_distance23 = np.sqrt( np.square(np.abs(positions[1][0] - positions[0][0])) + np.square(np.abs(positions[1][1] - positions[0][1])))
        print("newuwb 13 | 23", new_uwb_distance13, " | ", new_uwb_distance23)
        if len(currentMoves) == 9:
            print("out of moves at position: ", positions)
            movement = np.random.choice([-2, 0, 2], size=2)                
            dfound_x = movement[0]
            dfound_y = movement[1]
            currentMoves.clear()
        elif np.abs(new_uwb_distance13 - 10) + np.abs(new_uwb_distance23 - 5) < np.abs(uwb_distance13 - 10) + np.abs(uwb_distance23 - 5):
            #new distance is closer, continue moving in that direction
            dfound_x = movement[0]
            dfound_y = movement[1]
            currentMoves.clear()
            ...
            #go back, try another direction
            plt.scatter(positions[0, 0], positions[0, 1], c='red', label='Robot attempts this movement', marker=6)
            positions[0][0] = positions[0][0] - movement[0]
            positions[0][1] = positions[0][1] - movement[1]
            currentMoves.add((movement[0], movement[1]))
            #UNIMPORTANT NOTE: if we get farther away we can actually improve this algorithm reverse/slightly alter the directions, but its almost 11 and i wanna sleep
    ...
    plt.grid(True)
    plt.legend()
    plt.pause(0.2)  # Pause to simulate animation
print(positions)
plt.show()
